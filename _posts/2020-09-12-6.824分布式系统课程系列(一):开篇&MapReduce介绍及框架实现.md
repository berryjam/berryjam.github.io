...
- 另外为了防止worker运行中出现崩溃，导致写入的文件是不完整的，例如在处理Reduce任务的时候，每处理完一个key、value就写入到文件`mr-out-Y`，但是在写入所有数据之前突然崩溃，那么这个`mr-out-Y`文件肯定是不完整。MapReduce论文提到了，使用临时文件写入结果再通过原子重命名方式来保证一旦`mr-out-Y`生成了，内容就是完整的。代码逻辑如下：
```
